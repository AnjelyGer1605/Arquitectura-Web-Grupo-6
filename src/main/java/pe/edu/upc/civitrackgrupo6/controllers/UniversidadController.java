...
public class UniversidadController {
    ...
    public ResponseEntity<?> universidadesConMasEstudiantes() {
        List<Object[]> datos = uS.universidadesConMasEstudiantes();

        List<Map<String, Object>> respuesta = datos.stream().map(obj -> {
            Map<String, Object> map = new HashMap<>();
            map.put("nombreUniversidad", obj[0]);
            map.put("totalEstudiantes", obj[1]);
            return map;
        }).collect(Collectors.toList());
