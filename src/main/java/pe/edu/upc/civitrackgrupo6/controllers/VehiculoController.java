...
public class VehiculoController {
    ...
    public void insertar(@RequestBody VehiculoDTO dto) {
        ...
        Conductor conductor = cS.listId(Long.valueOf(dto.getIdConductor()));
    ...
    public ResponseEntity<String> modificar(@RequestBody VehiculoDTO dto) {
        ...
        if (existente == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("No existe un vehiculo con el ID: " + dto.getIdVehiculo());
        }
    ...
    public ResponseEntity<?> listarVehiculosActivos() {
        ...
        if (lista.isEmpty()) {
            return ResponseEntity.status(HttpStatus.OK)
                    .body("No hay vehículos activos registrados.");
        }
    ...
    public ResponseEntity<?> listarPorConductor(@PathVariable("idConductor") Long idConductor) {
        List<VehiculoDTO> lista = vS.findVehiculosByConductor(idConductor).stream().map(x -> {
            ModelMapper m = new ModelMapper();
            return m.map(x, VehiculoDTO.class);
        }).collect(Collectors.toList());
        ...
        if (lista.isEmpty()) {
            return ResponseEntity.status(HttpStatus.OK)
                    .body("No hay vehículos registrados para el conductor con ID: " + idConductor);
        }
